...
```{r}
Weekly5$date <- as.Date(Weekly5$start_date, format = "%m-%d-%Y")
```

```{r}
Weekly5$month_year <- format(Weekly5$start_date, "%b-%Y")
```
...
Weekly5 %>% 
  ...
  geom_line()+
  scale_x_continuous(  # Row numbers as x-axis positions
    breaks=seq(1, nrow(df), by = 4)
    labels = Weekly5$month_year)
```
